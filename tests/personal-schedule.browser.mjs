...
try {
  ...
  const scopeCard=page.locator('[data-schedule-id="9002"]');
  await scopeCard.click();
  await page.locator('input[name=title]').fill('只改这次');
  await page.locator('.personal-edit-modal .save').click();
  const scopeDialog=page.getByRole('dialog',{name:'修改重复日程',exact:true});
  await scopeDialog.waitFor();
  assert.equal(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'][1].title),'重复修改验证');
  await scopeDialog.getByRole('button',{name:/仅修改本次/}).click();
  assert.deepEqual(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'].map(t=>t.title)),['重复修改验证','只改这次','重复修改验证','重复修改验证']);
  await scopeCard.click();
  await page.locator('input[name=title]').fill('本次及以后改名');
  await page.locator('textarea[name=note]').fill('仅本次的新总结');
  await page.locator('.personal-edit-modal .save').click();
  ...
  assert.equal(await page.locator('input[name=title]').inputValue(),'取消的草稿');
  assert.equal(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'][1].title),'本次及以后改名');
  await page.locator('.personal-edit-modal > header button').click();
  const scopeHandle=scopeCard.locator('[data-resize-edge="end"]');
  await scopeHandle.scrollIntoViewIfNeeded();
  const scopeHandleBox=await scopeHandle.boundingBox();const scopeTrackBox=await scopeCard.locator('..').boundingBox();
  await page.mouse.move(scopeHandleBox.x+scopeHandleBox.width/2,scopeHandleBox.y+scopeHandleBox.height/2);await page.mouse.down();
  await page.mouse.move(scopeHandleBox.x+scopeHandleBox.width/2,scopeTrackBox.y+3.5/15*scopeTrackBox.height,{steps:8});await page.mouse.up();
  await scopeDialog.waitFor();
  ...
  await planDialog.getByRole('tab',{name:'周一'}).press('ArrowRight');
  await planDialog.getByRole('textbox',{name:'周二计划',exact:true}).fill('周二的草稿');
  await planDialog.getByRole('tab',{name:'周一'}).click();
  assert.equal(await planDialog.getByRole('textbox',{name:'周一计划',exact:true}).inputValue(),'准备第一节课');
  ...
  await page.getByRole('button',{name:'计划',exact:true}).click();
  assert.equal(await planDialog.getByRole('textbox',{name:'本周计划',exact:true}).inputValue(),'本周集中完成课程');
  await planDialog.getByRole('tab',{name:'周三'}).click();
