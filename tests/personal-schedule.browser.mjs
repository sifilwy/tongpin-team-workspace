...
try {
  ...
  const y = hour => bounds.y + (hour - 7) / 19 * bounds.height;
  ...
  const y2 = hour => nextBounds.y + (hour - 7) / 19 * nextBounds.height;
  ...
  const resizeTo=async(edge,hour)=>{
    ...
    await page.mouse.move(box.x+box.width/2,trackBox.y+(hour-7)/19*trackBox.height,{steps:8});
  };
  ...
  await page.mouse.move(shortBox.x+shortBox.width/2,moveTrack.y+7/19*moveTrack.height+shortBox.height/2,{steps:8});
  ...
  assert.ok(Math.abs(previewBox.height-moveTrack.height/76)<1,'Move preview must show the full 15-minute range');
  await page.mouse.up();
  ...
  await page.mouse.move(scopeHandleBox.x+scopeHandleBox.width/2,scopeTrackBox.y+3.5/19*scopeTrackBox.height,{steps:8});await page.mouse.up();
  await scopeDialog.waitFor();
  ...
  assert.equal(await pendingEdit.getByRole('button',{name:'保存修改',exact:true}).count(),0);
  await page.waitForFunction(()=>window.fixture['tongpin-personal-tasks-v3'].some(task=>task.title==='修改后的待安排' && task.category==='皮球' && task.note==='第一行备注\n第二行备注'));
  assert.equal(await pendingEdit.isVisible(),true);
  await pendingEdit.getByRole('button',{name:'关闭',exact:true}).click();
  ...
  await pendingEdit.getByRole('textbox',{name:'任务名称',exact:true}).fill('   ');
  await pendingEdit.getByRole('button',{name:'关闭',exact:true}).click();
  await pendingEdit.getByRole('alert').waitFor();
  assert.equal(await pendingEdit.isVisible(),true);
  await pendingEdit.getByRole('textbox',{name:'任务名称',exact:true}).fill('关闭时的修改');
  await page.keyboard.press('Escape');
  await pendingEdit.waitFor({state:'hidden'});
  assert.equal(await planDialog.isVisible(),true);
  assert.equal(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'].some(task=>task.title==='关闭时的修改')),true);
  await planDialog.getByRole('button',{name:'关闭计划',exact:true}).click();
  console.log('PASS: pending edits autosave without a save button, preserve plan draft, reject empty titles, flush latest changes on close/Escape');
  await page.clock.install({time:new Date(2026,8,10,13,25)});
  await page.evaluate(()=>{
    window.fixture['tongpin-personal-tasks-v3']=[{id:9800,title:'跨午夜的日程',owner:'xzx',due:'2026-09-10',done:false,category:'独立',note:'凌晨前完成',startTime:'23:00',endTime:'01:00'}];
    window.renderSchedule();
  });
  const view=page.getByRole('combobox',{name:'日程显示范围',exact:true});
  await view.waitFor();
  assert.equal(await page.locator('.personal-day-track').count(),7);
  assert.match(await page.locator('.personal-time-axis').innerText(),/22:00[\s\S]*23:00[\s\S]*次日 00:00[\s\S]*次日 01:00[\s\S]*次日 02:00/);
  await view.selectOption('3');
  assert.deepEqual(await page.locator('.personal-day-track').evaluateAll(nodes=>nodes.map(node=>node.dataset.due)),['2026-09-10','2026-09-11','2026-09-12']);
  assert.deepEqual(await page.locator('.personal-date>header>span').allTextContents(),['周四','周五','周六']);
  await page.getByRole('button',{name:'后3天',exact:true}).click();
  assert.equal(await page.locator('.personal-day-track').first().getAttribute('data-due'),'2026-09-13');
  assert.equal(await page.locator('.personal-now-line').count(),0);
  await page.getByRole('button',{name:'今天',exact:true}).click();
  await view.selectOption('4');assert.equal(await page.locator('.personal-day-track').count(),4);
  const line=page.locator('.personal-now-line');
  assert.equal(await line.locator('..').getAttribute('data-due'),'2026-09-10');
  assert.match(await line.getAttribute('aria-label'),/13:25/);
  await page.clock.fastForward(60000);
  assert.match(await line.getAttribute('aria-label'),/13:26/);
  const overnight=page.locator('[data-schedule-id="9800"]');
  assert.match(await overnight.locator('time').innerText(),/23:00–次日 01:00/);
  const range=await overnight.evaluate(card=>({height:card.getBoundingClientRect().height,track:card.parentElement.getBoundingClientRect().height}));
  assert.ok(Math.abs(range.height+4-range.track*2/19)<1);
  await overnight.click();
  assert.equal(await page.locator('input[name=endTime]').inputValue(),'01:00');
  await page.locator('input[name=endTime]').fill('02:00');
  await page.locator('.personal-edit-modal .save').click();
  assert.equal(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'][0].endTime),'02:00');
  // Drag the end back through midnight; the saved range remains on its original date.
  const lateHandle=overnight.locator('[data-resize-edge=end]');
  await lateHandle.scrollIntoViewIfNeeded();
  const lateBox=await lateHandle.boundingBox();const lateTrack=await overnight.locator('..').boundingBox();
  await page.mouse.move(lateBox.x+lateBox.width/2,lateBox.y+lateBox.height/2);await page.mouse.down();
  await page.mouse.move(lateBox.x+lateBox.width/2,lateTrack.y+17.5/19*lateTrack.height,{steps:8});
  assert.match(await page.locator('.personal-preview-start').textContent(),/23:00–次日 00:30.*90/);
  await page.mouse.up();
  assert.equal(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'][0].endTime),'00:30');
  await overnight.click();
  await page.locator('input[name=endTime]').fill('03:00');await page.locator('.personal-edit-modal .save').click();
  await page.getByRole('alert').waitFor();
  assert.equal(await page.evaluate(()=>window.fixture['tongpin-personal-tasks-v3'][0].endTime),'00:30');
  await page.locator('.personal-edit-modal>header button').click();
  await page.clock.setSystemTime(new Date(2026,8,11,1,25));await page.evaluate(()=>window.dispatchEvent(new Event('focus')));
  await page.waitForFunction(()=>document.querySelector('.personal-now-line')?.getAttribute('aria-label').includes('次日 01:25'));
  assert.equal(await line.locator('..').getAttribute('data-due'),'2026-09-10');
  assert.ok(Math.abs(await line.evaluate(el=>parseFloat(el.style.top))-(1525-420)/1140*100)<0.01);
  await page.clock.setSystemTime(new Date(2026,8,11,3,0));await page.evaluate(()=>window.dispatchEvent(new Event('focus')));
  await page.waitForFunction(()=>!document.querySelector('.personal-now-line'));
  console.log('PASS: 07:00–next-day 02:00 axis, 3/4/7 days and navigation, correct weekdays, ticking subtle current-time line, overnight edit/resize/validation and date boundary');
