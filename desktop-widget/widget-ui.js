...
document.addEventListener('DOMContentLoaded', () => {
  ...
  style.textContent = 'body{padding-top:36px!important;box-sizing:border-box}body .desktop-agenda{padding:12px 16px 18px;min-height:calc(100vh - 36px)}body .desk-header{display:none}body .desk-top{margin-top:0}body .desk-footer a{display:none}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#bdcbe1;border-radius:5px}';
  ...
  shadow.innerHTML = `<style>
    ...
    *{box-sizing:border-box}header{height:36px;display:flex;align-items:center;background:#e8eef9;padding-left:16px}
  ...
  </style><header><strong>同屏</strong><button id="more" aria-label="更多选项" title="更多选项" aria-expanded="false">⋯</button><button id="collapse" data-command="collapse" aria-label="收起为小圆点" title="收起为小圆点">−</button><button id="close" data-command="close" aria-label="关闭组件" title="关闭组件">×</button></header>
  <div class="menu" hidden><button data-command="refresh">刷新日程</button><button data-command="move">移动日程位置</button><button data-command="reset">恢复右侧位置</button><button data-command="collapse">收起为小圆点</button><button data-command="close">退出组件</button></div>`;
  ...
  window.chrome.webview.addEventListener('message', event => {
    if (event.data === 'attached' || event.data === 'detached') shadow.querySelector('[data-command=move]').textContent = event.data === 'attached' ? '移动日程位置' : '固定到桌面';
  });
