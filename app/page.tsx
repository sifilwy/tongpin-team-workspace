...
export default function Page() {
  ...
  const [overviewMode, setOverviewMode] = useState<"overview" | "people">("overview");
  ...
  const [completedCategory, setCompletedCategory] = useState<Category>("运营");
  ...
  function updateTask(id: number, patch: Partial<Task>) {
    ...
    const effectivePatch = patch.status === "已完成" ? { completedAt: new Date().toISOString(), completedBy: patch.completedBy || "xzx", ...patch } : patch;
    ...
    if (patch.status === "已完成" && target && view !== "timeline") {
      ...
      setCompletedCategory(patch.category || target.category);
  ...
  function moveTimelineStatus(id: number, status: "待完成" | "已完成") {
    setTasks((current) => current.map((task) => task.id !== id ? task : status === "待完成"
      ? { ...task, owner: "待分配", status, completedAt: undefined, completedBy: undefined }
      : { ...task, status, completedAt: new Date().toISOString(), completedBy: task.completedBy || "xzx" }));
    setSelectedId(id);
  }
  ...
  function openCompletedList() {
    const newest = tasks.find((task) => task.status === "已完成");
    if (newest) setCompletedCategory(newest.category);
  ...
  function createTask(event: FormEvent<HTMLFormElement>) {
    ...
    const task: Task = { id: Date.now(), title, owner, category: String(data.get("category")) as Category, due: String(data.get("due")), status: owner === "待分配" ? "待完成" : "进行中", description: String(data.get("description") || ""), notionUrl: NOTION_COLLAB_URL, reviews: [] };
  ...
  return <main className="app-shell">
    ...
    {showNew && <div className="modal-layer"><form className="task-modal" onSubmit={createTask}><div className="modal-title"><div><strong>新建任务</strong><span>创建后进入待安排，也可以直接指定负责人</span></div><button type="button" onClick={() => setShowNew(false)}>×</button></div><label>任务名称<input name="title" autoFocus required placeholder="输入一件需要协作的事" /></label><div className="form-row"><label>负责人<select name="owner" defaultValue="xzx"><option>待分配</option>{members.map(member => <option key={member.name}>{member.name}</option>)}</select></label><label>任务分类<select name="category">{categories.map(category => <option key={category}>{category}</option>)}</select></label></div><label>计划日期<input name="due" type="date" defaultValue={iso(new Date())} required /></label><label>补充说明<textarea name="description" placeholder="说明结果、配合方式或注意事项" /></label><label>Notion 任务笔记<input name="notionUrl" type="url" placeholder="粘贴团队共享的 Notion 页面链接" /></label><button className="modal-submit">创建任务</button></form></div>}
...
function TaskRow({ task, onClick }: { task: Task; onClick: () => void }) { return <button className="task-row" onClick={onClick}><span className={`task-mark ${task.category}`} /><span><strong>{task.title}</strong><small>{task.owner} · {task.category} · {task.due.slice(5).replace("-", "/")}</small></span><em>打开</em></button>; }
...
function CalendarCard({ task, selected, dragging, dropPosition, onClick, onContextMenu, onDragStart, onDragEnd, onDragOver, onDrop }: { task: Task; selected: boolean; dragging: boolean; dropPosition: "before" | "after" | null; onClick: () => void; onContextMenu: (event: ReactMouseEvent, id: number) => void; onDragStart: (event: ReactDragEvent<HTMLButtonElement>, id: number) => void; onDragEnd: () => void; onDragOver: (event: ReactDragEvent<HTMLButtonElement>) => void; onDrop: (event: ReactDragEvent<HTMLButtonElement>) => void }) { return <button draggable className={`calendar-card ${task.category}${task.status === "已完成" ? " completed" : ""}${selected ? " active" : ""}${dragging ? " dragging" : ""}${dropPosition ? ` insert-${dropPosition}` : ""}`} onClick={onClick} onContextMenu={(event) => onContextMenu(event, task.id)} onDragStart={(event) => onDragStart(event, task.id)} onDragEnd={onDragEnd} onDragOver={onDragOver} onDrop={onDrop}><strong>{task.title}</strong><small>{task.owner}{task.status === "已完成" ? " · 已完成" : ""}</small></button>; }
...
function EditTaskModal({ task, onClose, onSave }: { task: Task; onClose: () => void; onSave: (patch: Partial<Task>) => void }) { function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const status = String(data.get("status")) as Status; onSave({ title: String(data.get("title")), owner: String(data.get("owner")), category: String(data.get("category")) as Category, due: String(data.get("due")), description: String(data.get("description")), notionUrl: String(data.get("notionUrl") || "").trim(), status, completedAt: status === "已完成" ? task.completedAt || new Date().toISOString() : undefined }); } return <div className="modal-layer"><form className="task-modal" onSubmit={submit}><div className="modal-title"><div><strong>编辑任务</strong><span>修改任务信息与右侧展示的任务说明</span></div><button type="button" onClick={onClose}>×</button></div><label>任务名称<input name="title" defaultValue={task.title} required /></label><div className="form-row"><label>负责人<select name="owner" defaultValue={task.owner}><option>待分配</option>{members.map(member => <option key={member.name}>{member.name}</option>)}</select></label><label>任务分类<select name="category" defaultValue={task.category}>{categories.map(category => <option key={category}>{category}</option>)}</select></label></div><div className="form-row"><label>计划日期<input name="due" type="date" defaultValue={task.due} required /></label><label>计划进度<select name="status" defaultValue={task.status}><option value="待完成">待安排</option><option value="进行中">正在进行</option><option value="已完成">已完成</option></select></label></div><label>任务说明<textarea name="description" defaultValue={task.description} placeholder="说明任务目标、完成标准和需要配合的事项" /></label><label>Notion 任务笔记<input name="notionUrl" type="url" defaultValue={task.notionUrl || ""} placeholder="粘贴团队共享的 Notion 页面链接" /></label><button className="modal-submit">保存修改</button></form></div>; }
