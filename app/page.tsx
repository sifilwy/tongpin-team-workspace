...
import { useMember } from "./components/TeamAccess";
...
import { AssistantPicker, BillingFields } from "./components/TaskExtras";
...
export default function Page() {
  ...
  const [tasks, setTasks] = useSharedState<Task[]>("tongpin-tasks-v8", () => initialTasks.map((task) => ({ ...task, notionUrl: NOTION_COLLAB_URL })));
  const [messages, setMessages] = useSharedState<Message[]>("tongpin-messages-v8", initialMessages);
  const [selectedId, setSelectedId] = useState(1);
  const [overviewMember, setOverviewMember] = useState(member);
  ...
  const [completion, setCompletion] = useState<{ id: number; patch: Partial<Task> } | null>(null);
  ...
  function updateTask(id: number, patch: Partial<Task>, confirmQuantity = false) {
    const target = tasks.find((task) => task.id === id);
    if (!confirmQuantity && target && patch.status === "已完成" && (Object.hasOwn(patch, "quantity") ? patch.quantity : target.quantity) === null && (patch.amount ?? target.amount) !== undefined) {
      setCompletion({ id, patch }); setTaskMenu(null); return;
    }
    const effectiveOwner = patch.owner || target?.owner || "xzx";
    const effectivePatch = patch.status === "已完成"
      ? { completedAt: target?.completedAt || new Date().toISOString(), completedBy: patch.completedBy || target?.completedBy || member, amountRecipient: target?.amountRecipient || (effectiveOwner === "待分配" ? "xzx" : effectiveOwner), ...patch }
      : patch.status
  ...
  function moveTimelineStatus(id: number, status: "待完成" | "已完成") {
    if (status === "已完成") { updateTask(id, { status }); setSelectedId(id); return; }
  ...
  function createTask(event: FormEvent<HTMLFormElement>) {
    ...
    const task: Task = { id: Date.now(), title, owner, category: String(data.get("category")) as Category, due: String(data.get("due")), status: owner === "待分配" ? "待完成" : "进行中", amount: optionalAmount(data.get("amount")), quantity: String(data.get("quantity") || "").trim() === "" ? null : Number(data.get("quantity")), assistants: data.getAll("assistants").map(String), description: String(data.get("description") || ""), notionUrl: NOTION_COLLAB_URL, reviews: [] };
  ...
  function postMessage(event: FormEvent) {
    ...
    setMessages((current) => [{ id: Date.now(), author: member, text: messageText.trim(), createdAt: preciseTime() }, ...current]); setMessageText("");
  }
  function addReview(event: FormEvent) {
    event.preventDefault(); if (!reviewText.trim()) return;
    updateTask(selected.id, { reviews: [...selected.reviews, { id: Date.now(), author: member, text: reviewText.trim(), createdAt: preciseTime() }] }); setReviewText("");
  }
  ...
  return <main className="app-shell">
    ...
    {view === "timeline" && <section className="timeline-page">
      ...
      <aside className="task-detail"><span className={`status-pill ${selected.status}`}>{selected.status}</span><h2>{selected.title}</h2>{selected.amount !== undefined && <div className="task-amount-chip">任务金额 <b>{selected.quantity === null ? "待确认" : `¥${taskTotal(selected)}`}</b><small>单次 ¥{selected.amount} · {selected.quantity === null ? "次数待确认" : `${selected.quantity ?? 1} 次`}</small></div>}<p>{selected.description || "暂无补充说明"}</p><div className="notion-task-link"><span>任务笔记</span>{selected.notionUrl ? <a href={selected.notionUrl} target="_blank" rel="noreferrer">打开 Notion ↗</a> : <button type="button" onClick={() => setEditTaskIdState(selected.id)}>绑定 Notion 页面</button>}</div><label>负责人<select value={selected.owner} onChange={(event) => updateTask(selected.id, { owner: event.target.value })}><option>待分配</option>{members.map(member => <option key={member.name}>{member.name}</option>)}</select></label><AssistantPicker value={selected.assistants || []} onChange={assistants => updateTask(selected.id, { assistants })} /><label>任务分类<select value={selected.category} onChange={(event) => updateTask(selected.id, { category: event.target.value as Category })}>{categories.map(category => <option key={category}>{category}</option>)}</select></label><label>计划日期<input type="date" value={selected.due} onChange={(event) => updateTask(selected.id, { due: event.target.value })} /></label>{selected.status === "待完成" && <button className="detail-action" onClick={() => updateTask(selected.id, { status: "进行中" })}>安排进时间线</button>}{selected.status === "进行中" && <button className="detail-action" onClick={() => updateTask(selected.id, { status: "已完成", completedAt: new Date().toISOString() })}>标记完成</button>}<section className="review-box"><PanelTitle title="沟通" note="每条沟通保留署名" />{selected.reviews.map(review => <article key={review.id}><div><strong>{review.author}</strong><time>{review.createdAt}</time></div><p>{review.text}</p></article>)}<form onSubmit={addReview}><select disabled value={reviewAuthor} onChange={(event) => setReviewAuthor(event.target.value)}>{members.map(member => <option key={member.name}>{member.name}</option>)}</select><textarea value={reviewText} onChange={(event) => setReviewText(event.target.value)} placeholder="写下沟通或补充…" /><button>发送</button></form></section></aside>
    </section>}
    ...
    {completion && (() => { const task = tasks.find(item => item.id === completion.id); if (!task) return null; return <div className="modal-layer"><form className="task-modal" role="dialog" aria-label="确认实际产出" onSubmit={event => {
      event.preventDefault(); const data = new FormData(event.currentTarget);
      updateTask(completion.id, { ...completion.patch, amount: optionalAmount(data.get("amount")), quantity: Number(data.get("quantity")) }, true); setCompletion(null);
...
function OverviewTaskDetail({ task, onClose, onEdit, onAddNote }: { task: Task; onClose: () => void; onEdit: () => void; onAddNote: (note: TaskNote) => void }) {
  ...
  function submitNote(event: FormEvent) {
    event.preventDefault();
    if (!text.trim()) return;
    onAddNote({ id: Date.now(), author: member, text: text.trim(), createdAt: preciseTime() });
    setText("");
  }
  return <section className="overview-task-detail"><header><div><div className="detail-overline"><span>任务说明</span><time>{task.due.replaceAll("-", "/")}</time></div><div className="detail-title-row"><strong>{task.title}</strong><span className={`detail-category ${task.category}`}>{task.category}</span><span className="detail-status">{task.status === "待完成" ? "待安排" : task.status === "进行中" ? "正在推进" : "已完成"}</span></div></div><button aria-label="关闭任务说明" onClick={onClose}>×</button></header><div className="task-assist-note">负责人：{task.owner}{task.assistants?.length ? ` · 协助：${task.assistants.join("、")}` : ""}</div><div className="task-description-top"><span className="description-label">说明</span><p>{task.description || "还没有补充任务说明。"}</p></div><div className="notion-task-link"><span>任务笔记</span>{task.notionUrl ? <a href={task.notionUrl} target="_blank" rel="noreferrer">打开 Notion ↗</a> : <button type="button" onClick={onEdit}>绑定 Notion 页面</button>}</div><div className="task-conversation"><div className="conversation-title"><strong>任务讨论</strong><span>{([...task.reviews, ...(task.notes || [])].filter((note, index, all) => all.findIndex(n => n.id === note.id) === index)).length} 条</span></div><div className="task-note-list">{([...task.reviews, ...(task.notes || [])].filter((note, index, all) => all.findIndex(n => n.id === note.id) === index)).map((note) => { const noteMember = members.find((item) => item.name === note.author); return <article key={note.id}><i className="avatar" style={{ background: noteMember?.color || "#8b8f9b" }}>{note.author[0]}</i><div><header><strong>{note.author}</strong><time>{note.createdAt}</time></header><p>{note.text}</p></div></article>; })}</div><form className="task-note-form" onSubmit={submitNote}><div><select disabled value={author} onChange={(event) => setAuthor(event.target.value)}>{members.map((item) => <option key={item.name}>{item.name}</option>)}</select><button type="button" onClick={onEdit}>编辑说明</button></div><div className="note-composer"><textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="补充进度、问题或需要谁配合…" /><button type="submit">发送</button></div></form></div></section>;
}
...
function EditTaskModal({ task, onClose, onSave }: { task: Task; onClose: () => void; onSave: (patch: Partial<Task>) => void }) { function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const status = String(data.get("status")) as Status; onSave({ title: String(data.get("title")), owner: String(data.get("owner")), category: String(data.get("category")) as Category, due: String(data.get("due")), amount: optionalAmount(data.get("amount")), quantity: String(data.get("quantity") || "").trim() === "" ? null : Number(data.get("quantity")), assistants: data.getAll("assistants").map(String), description: String(data.get("description")), notionUrl: String(data.get("notionUrl") || "").trim(), status, completedAt: status === "已完成" ? task.completedAt || new Date().toISOString() : undefined }); } return <div className="modal-layer"><form className="task-modal" onSubmit={submit}><div className="modal-title"><div><strong>编辑任务</strong><span>修改任务信息、金额与右侧展示的任务说明</span></div><button type="button" onClick={onClose}>×</button></div><label>任务名称<input name="title" defaultValue={task.title} required /></label><div className="form-row"><label>负责人<select name="owner" defaultValue={task.owner}><option>待分配</option>{members.map(member => <option key={member.name}>{member.name}</option>)}</select></label><label>任务分类<select name="category" defaultValue={task.category}>{categories.map(category => <option key={category}>{category}</option>)}</select></label></div><AssistantPicker defaultValue={task.assistants || []} /><div className="form-row"><label>计划日期<input name="due" type="date" defaultValue={task.due} required /></label><label>计划进度<select name="status" defaultValue={task.status}><option value="待完成">待安排</option><option value="进行中">正在进行</option><option value="已完成">已完成</option></select></label></div><BillingFields amount={task.amount} quantity={task.quantity === undefined ? (task.amount === undefined ? null : 1) : task.quantity} /><label>任务说明<textarea name="description" defaultValue={task.description} placeholder="说明任务目标、完成标准和需要配合的事项" /></label><label>Notion 任务笔记<input name="notionUrl" type="url" defaultValue={task.notionUrl || ""} placeholder="粘贴团队共享的 Notion 页面链接" /></label><button className="modal-submit">保存修改</button></form></div>; }
