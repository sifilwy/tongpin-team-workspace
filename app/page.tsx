...
export default function Page() {
  ...
  return <main className="app-shell">
    ...
    {showNew && <div className="modal-layer"><form className="task-modal" onSubmit={createTask}><div className="modal-title"><div><strong>新建任务</strong><span>创建后进入待安排，也可以直接指定负责人</span></div><button type="button" onClick={() => setShowNew(false)}>×</button></div><label>任务名称<input name="title" autoFocus required placeholder="输入一件需要协作的事" /></label><div className="form-row"><label>负责人<select name="owner" defaultValue={member}><option>待分配</option>{members.map(member => <option key={member.name}>{member.name}</option>)}</select></label><label>任务分类<select name="category">{categories.map(category => <option key={category}>{category}</option>)}</select></label></div><div className="form-row"><label>计划日期<input name="due" type="date" defaultValue={iso(new Date())} required /></label><label>任务金额（选填）<input name="amount" type="number" min="0" step="0.01" inputMode="decimal" placeholder="不填则不计金额" /></label></div><label>补充说明<textarea name="description" placeholder="说明结果、配合方式或注意事项" /></label><label>Notion 任务笔记<input name="notionUrl" type="url" placeholder="粘贴团队共享的 Notion 页面链接" /></label><button className="modal-submit">创建任务</button></form></div>}
