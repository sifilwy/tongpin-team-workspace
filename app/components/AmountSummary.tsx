...
export default function AmountSummary({ tasks }: { tasks: Task[] }) {
  const completed = tasks.filter((task) => task.status === "已完成" && taskTotal(task) > 0);
  const rows = members.map((member) => {
    const credited = completed.filter((task) => (task.amountRecipient || task.owner) === member.name);
    return {
      ...member,
      total: credited.reduce((sum, task) => sum + Math.round(taskTotal(task) * 100), 0) / 100,
      tasks: credited,
    };
  });
  ...
  return <section className="amount-summary">
    <header className="amount-summary-heading">
      <div><span>已完成任务</span><strong>金额汇总</strong><small>完成并确认实际次数后计入负责人；协助人不分摊金额。</small><small>{tasks.filter(task => task.status === "已完成" && task.amount !== undefined && task.quantity === null).length} 项任务次数待确认，暂不计入</small></div>
    ...
    <div className="amount-member-grid">
      {rows.map((member, index) => <article className={index === 0 ? "leading" : ""} key={member.name}>
        ...
        {member.tasks.length > 0 && <div className="amount-task-list">{member.tasks.map((task) => <div key={task.id}><span>{task.title}<small> · ¥{money.format(task.amount || 0)} × {task.quantity ?? 1} 次</small></span><b>¥{money.format(taskTotal(task))}</b></div>)}</div>}
      </article>)}
    </div>
  </section>;
}
